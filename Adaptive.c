...
// Configuration Adaptive predictive control
const double Ref_Meters = 180.0;	// Reference for meters in adaptive predictive control
double NL = 0.1;					// Noise Level for Adaptive Mechanism.
double GainA = 1.0;					// Gain for Adaptive Mechanism A
double GainB = 1.0;					// Gain for Adaptive Mechanism B
double MaxOut_PWM;
...
void adaptive(double *sp, double *t, double *y, double *u, double *yp, int max_out){
	...
	if (ini >= (PmA+10)){			// Reload for initialize  input/output vector	
		...
		u[0] = (int16_t)((y_dk - e1[hz-1] * y[0] - e2[hz-1] * y[1] - g2[hz-1] * u[1] - g3[hz-1] * u[2]) / h);		// Calculated Regulator Out
